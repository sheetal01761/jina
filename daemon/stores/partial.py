                    ...
                    if runtime_cls in ['ZEDRuntime', 'ContainerRuntime'] + list(
                        ...
                        if port_mapping:
                                ...
                                if pea_args.name in port_mapping.pea_names:
                                    for port_name in Ports.__fields__:
                                        if hasattr(pea_args, port_name):
                                            ...
                                            print(
                                                f'setting attr for {pea_args.name} {port_name}: \t{_getattr}'
                                            )
                                            setattr(
                                                pea_args,
                                                port_name,
                            ...
                            for replica in pod.replicas:
                                replica.args.port_in = pod.head_args.port_out
                                replica.args.port_out = pod.tail_args.port_in
                            print(
                                f'\n\nhead_args: \n{pod.head_args},\n\ntail_args: \n{pod.tail_args}\n'
                            )
